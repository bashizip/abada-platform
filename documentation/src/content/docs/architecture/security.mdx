...
| Workers | `worker:execute` | `abada-worker` | Use worker protocol v1; the first-party Agent Worker self-registers global capabilities |
| Insight | `insight:read`, `insight:review`, `insight:configure` | Project `REVIEWER` role plus a named approval lane | Read proposals/policies; approve or reject; update policies |
| Administration | all domains | `abada-admin` | Full engine access (cannot satisfy an Insight approval lane) |
