...
external production identity and optional best-effort telemetry. Runtime
command API v1, worker protocol v1 and database execution semantics are
unchanged.

The public `GET /api/v1/info` response is now a typed service-discovery
contract. It reports product and release identity, the documented BPMN subset,
PostgreSQL production persistence, API documentation links and authoritative
health-probe links. The earlier untyped `status`, `profile`, host/JVM runtime
details and DMN/CMMN roadmap claims have been removed. Clients that treated
this informational response as a health check must migrate to the advertised
Actuator liveness or readiness probe.

## Validation scope

This is a published evaluation release candidate. Its publication evidence consists of the
...
[1.1 RC roadmap](https://github.com/bashizip/abada-engine/blob/dev/docs/development/roadmap-to-1.1.0-rc.md).
Do not interpret this release candidate as a production availability or
hosted-SLA claim.

## Container architecture limitation

The original `1.0.0-rc.1` engine, Tenda and Orun image manifests contain
`linux/amd64` only. On an ARM64 Docker host, the repository launcher detects
these exact image tags and uses Docker's `amd64` compatibility mode. This
preserves the immutable published image digests while restoring a working
development launch on Apple Silicon and other ARM64 systems.

The publication workflow now builds `linux/amd64` and `linux/arm64` variants
and validates both manifest entries. Subsequent Abada release images are not
publishable unless both architectures are present.
